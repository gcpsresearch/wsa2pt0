...
if(max(lrRFE$results$ROC) >= .80) {
  ...
  rm2 <- train[, which(unlist(lapply(names(train), function(x) 
    grepl('schl_|_from|zoned|pct_|schlAtt_|schlEnr_|mob_H1|schlFRL|percentEnrolled|ss_|fg_|fsl_|sei', x, ignore.case = TRUE))))]
  ...
  go.mxs <- apply(as.data.frame(glmData[complete.cases(glmData), 1:length(go.coeffs) - 1]), 2, function(x) 
	...
	# set any GPA variables to just 1 SD above mean
		go.mxs[grepl("GPA", names(go.mxs))] <- go.mns[grepl("GPA", names(go.mns))] + .75*go.sds[grepl("GPA", names(go.sds))]
