...
if(p.grd == max(grds)) {
	dfm <- merge(dfm, nsc.model, by = "id", all.x =  TRUE)
	dfm$target <- dfm$p.e
}
...
if(p.grd == max(grds)){						
	 sink(file = paste(maindir, "\\results\\perc.mtg.target_", p.grd, 
		"th_", ps.m[p.grd + 1, 3], run, ".txt", sep = ""),
...
if(p.grd != max(grds)) {
save.image(paste0(maindir, "/data/prep/", p.grd, ps.m[p.grd + 1, 3], run, "wsa2pt0.RData"))
}
...
library(corrplot)
pdf(paste0(maindir, "\\results\\graphs\\", p.grd, "th", ps.m[p.grd + 1, 3], run, 
          "_corrPlot.pdf"), 
    width = 17, height = 20)
corrplot(predCor,
         order = "hclust", 
...
for(i in 1:length(unique(tuningPsSet[, 1]))) {
  ...
  if(ps[1, 2] != "") {
    sink(file = paste0(maindir, "\\results\\", p.grd, "th_", ps.m[p.grd + 1, 3], 
                        "_", as.character(ps[1, 1]), "_FullTuned", run, ".txt"),
         append = FALSE, split = TRUE)
  print(mFull)
  sink()
  }
  ...
  png(filename = paste(maindir, "\\results\\graphs\\", p.grd, "th_", ps.m[p.grd + 1, 3], 
                       "_", ps[1,1], "_ProbSuccess", run, ".png", sep = ""),
...
pdf(file = paste(maindir, "\\results\\graphs\\", p.grd, "th_", ps.m[p.grd + 1, 3], 
                 "_ROC_AUC_comparison", run, ".pdf", sep = ""),
    width = 8, height = 6)
...
for (l in 2:length(wrappers[, 1])) {  # could dynamically code wrappers to match fs.models and make this length(fs.models)
  ...
  pdf(file = paste(maindir, "\\results\\graphs\\", p.grd, "th_", ps.m[p.grd + 1, 3], 
                   "_", wrappers[l, 1], "_RFE", run, ".pdf", sep = ""),
      width = 10, height = 7)
...
if(p.grd < 12) {bal.f <- bal[bal$probability >= .60, ]}
  ...
  write.table(bal.f, paste0("..\\student.success.factor\\data\\prep\\bal.f", p.grd-1, "th_", ps.m[p.grd, 3], ".csv"), 
 	sep = ",", col.names=TRUE, row.names=FALSE)
  write.table(go.coeffs, paste0("..\\student.success.factor\\data\\prep\\go.coeffs", p.grd-1, "th_", ps.m[p.grd, 3], ".csv"), 
...
save.image(paste0("S:/Superintendent/Private/Strategy & Performance/", 
                  ...
                  "prep/", p.grd, "th", run, ps.m[p.grd + 1, 3], "_", 
                  sys.time, ".RData"))
}
