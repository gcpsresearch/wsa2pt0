...
cd "${ssipath}"
	...
	global		histyr5		"09"		// historic year 5 - needed for CRCTs
...
log using code\logfiles\merge_all_es_ms_20${evalyr}.smcl, replace
	
	use data/prep/20${evalyr}/hs_model_20${evalyr}, clear
	keep if inlist(grade_${histyr1}${evalyr}, 8,7,6,5,4,3)
...
drop if startyear_grade_E ==. 
		outsheet using data/prep/20${evalyr}/7th8th_model_only_20${evalyr}.csv, c replace
restore		

preserve 
keep if grade_E ==7
drop if startyear_grade_E ==. 
		outsheet using data/prep/20${evalyr}/6th7th_model_only_20${evalyr}.csv, c replace
restore	
...
keep if grade_E ==6
drop if startyear_grade_E ==. 
		outsheet using data/prep/20${evalyr}/5th6th_model_only_20${evalyr}.csv, c replace
restore	

preserve 
keep if grade_E ==5
drop if startyear_grade_E ==. 
		outsheet using data/prep/20${evalyr}/4th5th_model_only_20${evalyr}.csv, c replace
restore	

preserve 
keep if grade_E ==4
drop if startyear_grade_E ==. 
		outsheet using data/prep/20${evalyr}/3rd4th_model_only_20${evalyr}.csv, c replace
restore	
}	 
...
*****************
	...
	merge 1:1 id using data/prep/20${evalyr}/mobility${histyr2}${histyr1}_es_ms, nogen keep (1 3)
