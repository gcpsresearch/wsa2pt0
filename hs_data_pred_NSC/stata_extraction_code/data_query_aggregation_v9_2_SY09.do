...
cd "${ssipath}"
	...
	global		nxtyr		"10"		// year beyond evaluated
	global		evalyr		"09"		// evaluated year
	global		histyr1		"08"		// historic year 1
	global		histyr2		"07"		// historic year 2
	global 		histyr3		"06"		// historic year 3
	global		histyr4		"05"		// historic year 4
	global		histyr5		"04"		// historic year 5 - needed for CRCTs
...
${save} "C:\Users\e201401047\Desktop\org\attend\attend`yr'", replace 
...
preserve
	foreach su in 9TH AME BIO ECON ALG GEOM PHY USH {
		...
		"`crt_pth'\20${histyr1}\SP${histyr1}_state_`su'.xls", ///
			sheet("EOCT_SP${histyr1}_`su'_State") cellrange(B2:H3) firstrow clear
	...
	use `eoct_9TH_ga_$histyr1', clear
		append using `eoct_AME_ga_$histyr1'
		append using `eoct_BIO_ga_$histyr1'
		append using `eoct_ECON_ga_$histyr1'
		append using `eoct_ALG_ga_$histyr1'
		append using `eoct_GEOM_ga_$histyr1'
...
preserve
	foreach su in 9TH AME BIO ECON ALG GEOM PHY USH{
		...
		"`crt_pth'\20${evalyr}\SP${evalyr}_state_`su' with SD.xls", ///
			sheet("EOCT_SP${evalyr}_`su'_State") cellrange(B2:G3) firstrow clear
				keep MeanScaleScore StDev
				rename(MeanScaleScore StDev) ///
					  (mn sd) 
			gen subject = "`su'"
...
collapse (mean)ss_9TH z_9TH ss_ALG z_ALG ss_AME z_AME ss_BIO z_BIO  ///
 ss_ECO z_ECO ss_GEO z_GEO ss_PHY z_PHY ss_USH z_USH, by(id)
...
preserve
	...
	* school size, % white
	...
	merge m:m SchoolNumb using "${ssipath}/data/prep/20${evalyr}/stateNum_to_gcpsNum_link", nogen keep(1 3)
		assert loc != .
		isid loc
	collapse (sum) W TotalEnrollment, by(loc)
		gen schl_percWht_H1 = W/TotalEnrollment
...
use data/orig/20${evalyr}/histyr_hs, clear
	...
	* summarize prior crct score by school
	...
	save data/prep/20${evalyr}/hs_model, replace

	use data/prep/20${evalyr}/hs_model, clear
