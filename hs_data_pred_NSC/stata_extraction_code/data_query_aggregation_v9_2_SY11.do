...
cd "${ssipath}"
	...
	global		nxtyr		"12"		// year beyond evaluated
	global		evalyr		"11"		// evaluated year
	global		histyr1		"10"		// historic year 1
	global		histyr2		"09"		// historic year 2
	global 		histyr3		"08"		// historic year 3
	global		histyr4		"07"		// historic year 4
	global		histyr5		"06"		// historic year 5
...
**************************
	...
	#delimit cr
		...
		gen pre = "667"
gen snum = string((SchoolNumb), "%04.0f")
tostring SchoolNumb, replace
replace SchoolNumb = pre+snum
drop snum pre
destring SchoolNumb , replace
...
preserve
	foreach su in 9TH AME BIO ECON ALG GEOM PHY USH {
		...
		"`crt_pth'\20${histyr2}\SP${histyr2}_state_`su' with SD.xls", ///
			sheet("EOCT_SP${histyr2}_`su'_State") cellrange(B2:H3) firstrow clear
	...
	use `eoct_9TH_ga_$histyr2', clear
		append using `eoct_AME_ga_$histyr2'
		append using `eoct_BIO_ga_$histyr2'
		append using `eoct_ECON_ga_$histyr2'
		append using `eoct_ALG_ga_$histyr2'
		append using `eoct_GEOM_ga_$histyr2'
		append using `eoct_PHY_ga_$histyr2'
		append using `eoct_USH_ga_$histyr2'
		
		replace subject = "GEO" if subject == "GEOM"
...
preserve
	foreach su in 9TH AMLIT BIO ECO MAI MAII PHY USH{
		...
		"`crt_pth'\20${histyr1}\SP${histyr1}_state_`su'.xls", ///
			sheet("EOCT_SP${histyr1}_`su'_State") cellrange(B2:G3) firstrow clear
...
preserve
	foreach su in 9TH AMLIT BIO ECON MAI MAII PHY USH {
		import excel using /// 
		"`crt_pth'\20${evalyr}\EOCT_SP20${evalyr}_State_Summaries\SP${evalyr}_state_`su' with SD.xls", ///
			sheet("SP${evalyr}_state_`su'") cellrange(B2:G3) firstrow clear
				keep MeanScaleScore StandardDeviation
				rename(MeanScaleScore StandardDeviation) ///
					  (mn sd) 
			gen subject = "`su'"
...
preserve
	...
	* school size, % white
	...
	merge m:m SchoolNumb using "${ssipath}/data/prep/20${evalyr}/stateNum_to_gcpsNum_link", nogen keep(1 3)
		assert loc != .
		isid loc
	collapse (sum) W_N TotalEnrollment, by(loc)
		gen schl_percWht_H1 = W_N/TotalEnrollment
