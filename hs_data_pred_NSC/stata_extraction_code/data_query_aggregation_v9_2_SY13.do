	...
	* now get summary by school vs. enrollment period
	...
	preserve
		...
		* save in place of mobility from Chris Lanzi attendance query
		save data/prep/20${evalyr}/sasi_mobility${histyr2}${histyr1}_hs, replace
	restore
...
**************************************************
insheet using "data/orig/20${evalyr}/EOCT_GRAD_REQ.csv", c clear
...
odbc load, clear exec("
					...
					outsheet using data/orig/20${evalyr}/psat${histyr2}${histyr1}.csv, c replace
...
use data/orig/20${evalyr}/histyr_hs, clear
	...
	* historic year grades < 9
	...
	replace pabs_${histyr2}${histyr1} =. ///
		if daysabsent_${histyr2}${histyr1} > daysenrolled_${histyr2}${histyr1} 
	assert pabs_${histyr2}${histyr1} <= 1 if pabs_${histyr2}${histyr1} !=.
 
	gen pabs_${histyr1}${evalyr} = ///
			daysabsent_${histyr1}${evalyr}/daysenrolled_${histyr1}${evalyr}
	replace pabs_${histyr1}${evalyr} =. ///
		if daysabsent_${histyr1}${evalyr} > daysenrolled_${histyr1}${evalyr} 
	assert pabs_${histyr1}${evalyr} <= 1 if pabs_${histyr1}${evalyr} !=.
	
	gen punex_${histyr2}${histyr1} = ///
			daysunexcused_${histyr2}${histyr1}/daysenrolled_${histyr2}${histyr1}
	replace punex_${histyr2}${histyr1} =. ///
		if daysunexcused_${histyr2}${histyr1} > daysenrolled_${histyr2}${histyr1} 
	assert punex_${histyr2}${histyr1} <= 1 if punex_${histyr2}${histyr1} !=.
			
	gen punex_${histyr1}${evalyr} = ///
			daysunexcused_${histyr1}${evalyr}/daysenrolled_${histyr1}${evalyr}
	replace punex_${histyr1}${evalyr} =. ///
		if daysunexcused_${histyr1}${evalyr} > daysenrolled_${histyr1}${evalyr} 
	assert punex_${histyr1}${evalyr} <= 1 if punex_${histyr1}${evalyr} !=.
