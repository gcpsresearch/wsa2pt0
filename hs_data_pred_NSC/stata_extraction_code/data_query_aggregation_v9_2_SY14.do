...
cd "${ssipath}"
	...
	global		nxtyr		"15"		// year beyond evaluated
	global		evalyr		"14"		// evaluated year
	global		histyr1		"13"		// historic year 1
	global		histyr2		"12"		// historic year 2
	global 		histyr3		"11"		// historic year 3
	global		histyr4		"10"		// historic year 4
	global		histyr5		"09"		// historic year 5
...
if $extract==1	{
log using code/logfiles/extract.smcl_20${evalyr}, replace
...
save data/prep/20${evalyr}/stateNum_to_gcpsNum_link, replace
...
${save} data/orig/20${evalyr}/evalyr_hs, replace
...
${save} data/orig/20${evalyr}/absence_evalyr, replace
...
${save} data/orig/20${evalyr}/fay_evalyr_hs, replace
	...
	* kept most recent value to avoid duplicates (only 26 duplicates)
	...
	#delimit cr
		...
		isid id
			...
			save data/prep/20${evalyr}/diploma_types, replace
	restore
	
	merge 1:1 id using data/prep/20${evalyr}/diploma_types, nogen keep(1 3)
	gen grad = lastwithdrawal == "G" & grad_diploma_type == 1 & transferout == 0
	drop if transferout == 1
	
	save data/prep/20${evalyr}/grad, replace		
...
${save} data/prep/20${evalyr}/retgft04, replace
	...
	#delimit cr
		...
		rename gcps_studentid id
		isid id grade altschoolindicator repeatinggradeindicator
${save} data/orig/20${evalyr}/histyr_hs, replace
...
${save} data/orig/20${evalyr}/absence_histyr, replace
...
****************************************************
	...
	*select the date to school calendar linking table
	...
	foreach yr in 20$histyr1 20$evalyr {
		...
		#delimit cr
${save} data/prep/20${evalyr}/calendar`yr'hs, replace
	} //END FOREACH
*/
...
${save} data/prep/20${evalyr}/mobility${histyr2}${histyr1}_hs, replace
...
********************************************************************************
	...
	#delimit cr	
${save} data/orig/20${evalyr}/crct_hs, replace
		...
		* keep latest grade subject result 
		...
		isid id subject
			drop n max_n
${save} data/prep/20${evalyr}/crct_extr_hs, replace
		...
		*reshape into student by variables format
		...
		isid id
${save} data/prep/20${evalyr}/crct_final_hs, replace
...
${save} data/orig/20${evalyr}/gpa_hs, replace
...
*********************************
	...
	rename permnum id
${save} data/orig/20${evalyr}/promotion_hs, replace
...
${save} data/prep/20${evalyr}/discipline_enroll_20`yr'_hs, replace			
	} //END FOREACH yr
...
*********************************
	...
	#delimit cr
		...
		keep if regexm(test_name, "RET") != 1   // use retakes next year?
			...
			isid id subject
${save} data/orig/20${evalyr}/gwy${evalyr}, replace */
...
${save} data/orig/20${evalyr}/gwyPassEver, replace 
...
*********************************
	...
	*INVALID_IND - checked and only Y or N
	...
	${save} data/orig/20${evalyr}/hswtPassEver, replace 
...
//get from Datamart SASI; set to 8 to 3 year historical interval. from current date or fixed date
	...
	#delimit cr
		...
		rename permnum id
		drop _*
${save} data/prep/20${evalyr}/yearstograd, replace
...
preserve
	foreach su in 9TH AMLIT BIO ECON MAI MAII PHY USH {
		import excel using /// 
		"`crt_pth'\20${histyr2}\SP${histyr2}_state_`su' with SD.xls", ///
			sheet("SP${histyr2}_state_`su'") cellrange(B2:I3) firstrow clear
...
preserve
	foreach su in 9LIT AMLIT BIO ECO MTHI MTHII PHY USH CAL{
		import excel using /// 
		"`crt_pth'\20${histyr1}\SP${histyr1}_state_`su'.xls", ///
			...
			save `eoct_`su'_ga_$histyr1', replace
	             ...
	             use `eoct_9LIT_ga_$histyr1', clear
		append using `eoct_AMLIT_ga_$histyr1'
		append using `eoct_BIO_ga_$histyr1'
		append using `eoct_ECO_ga_$histyr1'
		append using `eoct_MTHI_ga_$histyr1'
		append using `eoct_MTHII_ga_$histyr1'
		append using `eoct_PHY_ga_$histyr1'
		append using `eoct_USH_ga_$histyr1'
		append using `eoct_CAL_ga_$histyr1'		
...
* evaluation year 1
	***************
		...
		* bring in values from GA DOE
preserve
	foreach su in 9LIT AMLIT BIO ECO AGE MTHII PHY USH CAL{
		import excel using /// 
		...
		append using `eoct_AGE_ga_$evalyr'
...
replace year = 20${histyr1} if year <= 20${histyr1}
gen yr = string(year)
gen subj = subject+ "_"+yr
drop year subject yr
rename (ss_tot z ) (ss_ z_ )
reshape wide ss_ z_, i(id) j(subj) string
 
save data/prep/20${evalyr}/eoct${histyr2}${evalyr}, replace
...
**************************************************
	#delimit ;
		...
		dsn(ODS_Prod_Assm) user(Research) pass(Research);
	#delimit cr
${outsheet} using data/orig/20${evalyr}/act${histyr4}${evalyr}.csv, c replace
		...
		replace subject = trim(subject)
		rename (stunumb scale_score school_year) (id score year)
...
drop test_key exam_admin_date corrected_report yr subject year
reshape wide score, i(id) j(subj) s

${save} data/prep/20${evalyr}/act${histyr4}${evalyr}, replace
...
odbc load, clear exec("
		...
		* collReady variable based upon collegeBoard grade-specific thresholds
		...
		replace psat_collReady = 1 if 	psat_scoreCR != . & psat_scoreCR >= 45 & ///
			...
			drop notTenth
			isid id
		save data/prep/20${evalyr}/psat${histyr2}${histyr1}, replace
...
odbc load, clear exec("
					...
					outsheet using data/orig/20${evalyr}/sat${histyr4}${evalyr}.csv, c replace
		...
		rename `var' `=lower("`var'")'
			...
			replace subject = trim(subject)
			rename (stunumb school_year)  (id year)
...
gen subj = subject+ "_"+yr
		...
		drop test_key exam_admin_date nonstand_ind yr year subject
			reshape wide score, i(id) j(subj) s
	
		save data/prep/20${evalyr}/sat${histyr4}${evalyr}, replace
...
log using code/logfiles/calc.smcl_20${evalyr}, replace
...
preserve
	...
	* school size, % white
	...
	merge m:m SchoolNumb using "${ssipath}/data/prep/20${evalyr}/stateNum_to_gcpsNum_link", nogen keep(1 3)
	drop if loc == .
		assert loc != . 
	...
	rename TotalEnrollment schlEnr_H1
		drop White
	save data/prep/20${evalyr}/schl_enr, replace
	...
	* school %esol, %sped
	...
	collapse (sum) TotalEnrollment, by(loc)
		merge 1:1 loc using data/prep/20${evalyr}/schl_enr, nogen keep(1 3)
		gen schl_percSPED_H1 = TotalEnrollment/schlEnr_H1
		keep loc schl_percSPED_H1
	save data/prep/20${evalyr}/schl_sped, replace	
	...
	save data/prep/20${evalyr}/schl_esol, replace	
	...
	* % FRPL
	...
	rename PercentEligibleFreeOrReduced schlFRL_H1
		keep loc schlFRL_H1
	save data/prep/20${evalyr}/schl_frl, replace
	...
	* % enrolled days attended
	...
	rename AverageDailyAttendance schlAtt_H1
		keep loc schlAtt_H1
	save data/prep/20${evalyr}/schl_att, replace
...
preserve
	...
	save data/prep/20${evalyr}/census, replace
restore
...
use data/prep/20${evalyr}/discipline_enroll_20`yr'_hs, clear
...
rename `v' `v'_`yr'
	...
	codebook drate
	save data/prep/20${evalyr}/discipline_enroll_`yr'_metric_final_hs, replace
...
*!! schoolyear is fall of academic year !!*
	...
	use data/prep/20${evalyr}/yearstograd, clear
		...
		* compute expected grade for use below
		...
		foreach yr in `hy4' `hy3' `hy2' `hy1' { 
			...
			drop yr*
				...
				tempfile grades
				save `grades', replace
				save data/prep/20${evalyr}/grades, replace
				


	* HSGPA + HS courses completed // Is GPA in high school courses
	use data/orig/20${evalyr}/gpa_hs, clear
	count
	rename permnum id
...
preserve
	...
	********************************
	...
	save data/prep/20${evalyr}/failCore_rigorInd, replace
restore
...
preserve
	...
	* first prep and get proportion of "course-completion" reqs met
		...
		//coreind is perfectly matched to coresubjectcode - so no need to keep it
		  ...
		  merge 1:1 id using data/prep/20${evalyr}/grades, nogen keep(1 3)
		...
		***********************		
         ...
         ***********************
                 keep if grade_20${histyr1} == 9
						 ...
						 save data/prep/20${evalyr}/coursesTowardGrad, replace
							...
							collapse (sum) att_cr=creditsattempted wtd_mrk, by(id)
								isid id
									...
									hist(`s'1SemGPA`yminus'`y')
											...
											drop att_cr wtd_mrk
											count
												save data/prep/20${evalyr}/`s'1SemGPA`yminus'`y'_hs, replace
						...
						preserve
								...
								isid id
									...
									hist(`s'CumulGPA`yminus'`y')
											...
											drop att_cr wtd_mrk
											count
												save data/prep/20${evalyr}/`s'CumulGPA`yminus'`y', replace
						...
						keep if schoolyear == `gpaYr' //after extracting historical data, pare down to evalyr
								...
								isid id
									...
									hist(`s'AnnualGPA`yminus'`y')
											...
											drop att_cr wtd_mrk
											count
												save data/prep/20${evalyr}/`s'AnnualGPA`yminus'`y', replace
...
use data/orig/20${evalyr}/promotion_hs, clear
reshape wide endyear_grade startyear_grade repeatinggradeind, i(id) j(schoolyear)
keep id startyear_grade20${evalyr}  
* don't need KK or PK
	destring startyear_grade20${evalyr}, replace force
	drop if startyear_grade20${evalyr} == . 

save data/prep/20${evalyr}/promotion_hs, replace
...
use data/orig/20${evalyr}/histyr_hs, clear
	drop ageseptember1st //need age Sept variable from evalyr (here 2011-12)
	merge 1:1 id using data/orig/20${evalyr}/evalyr_hs, nogen keep(1 3)
	merge 1:1 id using data/orig/20${evalyr}/absence_evalyr,nogen keep (1 3)
	merge 1:1 id using data/orig/20${evalyr}/absence_histyr,nogen keep (1 3)
	merge 1:1 id using data/prep/20${evalyr}/discipline_enroll_${histyr1}_metric_final_hs, nogen keep (1 3)
	merge 1:1 id using data/prep/20${evalyr}/discipline_enroll_${evalyr}_metric_final_hs, nogen keep (1 3)
	merge 1:1 id using data/prep/20${evalyr}/crct_final_hs, nogen keep (1 3)
	merge 1:1 id using data/prep/mobility${histyr2}${histyr1}_hs, nogen keep (1 3)
	merge 1:1 id using data/prep/20${evalyr}/yearstograd, nogen keep(1 3)
	
	
	//2011-12 GPA - 1Sem is outcome for 11th to 12th analyses
	merge 1:1 id using data/prep/20${evalyr}/core1SemGPA20${histyr1}20${evalyr}_hs, nogen keep(1 3)
		merge 1:1 id using data/prep/20${evalyr}/lacourse1SemGPA20${histyr1}20${evalyr}_hs, nogen keep(1 3)
			merge 1:1 id using data/prep/20${evalyr}/macourse1SemGPA20${histyr1}20${evalyr}_hs, nogen keep(1 3)
			
	//2011-12 GPA - Annual is 2011-12 model output
	merge 1:1 id using data/prep/20${evalyr}/coreAnnualGPA20${histyr1}20${evalyr}, nogen keep(1 3)
		merge 1:1 id using data/prep/20${evalyr}/lacourseAnnualGPA20${histyr1}20${evalyr}, nogen keep(1 3)
			merge 1:1 id using data/prep/20${evalyr}/macourseAnnualGPA20${histyr1}20${evalyr}, nogen keep(1 3)
			
	//2010-11 GPA - Cumul is 2011-12 model input
	merge 1:1 id using data/prep/20${evalyr}/coreCumulGPA20${histyr2}20${histyr1}, nogen keep(1 3)
		merge 1:1 id using data/prep/20${evalyr}/lacourseCumulGPA20${histyr2}20${histyr1}, nogen keep(1 3)
			merge 1:1 id using data/prep/20${evalyr}/macourseCumulGPA20${histyr2}20${histyr1}, nogen keep(1 3)

	merge 1:1 id using data/prep/20${evalyr}/sei, nogen keep(1 3)
	merge 1:1 id using data/prep/20${evalyr}/coursesTowardGrad, nogen keep(1 3)
	merge 1:1 id using data/prep/20${evalyr}/sat${histyr4}${evalyr}, nogen keep(1 3)
	merge 1:1 id using data/prep/20${evalyr}/act${histyr4}${evalyr}, nogen keep(1 3)
	merge 1:1 id using data/prep/20${evalyr}/eoct${histyr2}${evalyr}, nogen keep(1 3)

	merge 1:1 id using data/prep/20${evalyr}/promotion_hs, nogen keep(1 3)
	merge 1:1 id using data/orig/20${evalyr}/fay_evalyr_hs, nogen keep(1 3)
	merge 1:1 id using data/prep/20${evalyr}/failcore_rigorInd, nogen keep(1 3)
	merge 1:1 id using data/prep/20${evalyr}/census, nogen keep(1 3)
	merge 1:1 id using data/prep/20${evalyr}/psat${histyr2}${histyr1}, nogen keep(1 3)
	merge 1:1 id using data/prep/20${evalyr}/grad, nogen keep(1 3)
		replace grad = 0 if grad == .
	merge 1:1 id using data/prep/20${evalyr}/retgft04, nogen keep(1 3)
	...
	* summarize prior crct score by school
	preserve
		collapse (mean) schl_totLA = ss_totLA schl_totMA = ss_totMA ///
						...
						by(loc)
					save data/prep/20${evalyr}/schl_crct, replace
	restore
	
			//add school-level variables
		merge m:1 loc using data/prep/20${evalyr}/schl_sped, nogen keep(1 3)
		merge m:1 loc using data/prep/20${evalyr}/schl_esol, nogen keep(1 3)
		merge m:1 loc using data/prep/20${evalyr}/schl_enr, nogen keep(1 3)
		merge m:1 loc using data/prep/20${evalyr}/schl_frl, nogen keep(1 3)
		merge m:1 loc using data/prep/20${evalyr}/schl_att, nogen keep(1 3)
		merge m:1 loc using data/prep/20${evalyr}/schl_crct, nogen keep(1 3)
		merge m:1 loc using data/prep/20${evalyr}/schl_sei, nogen keep(1 3)
	...
	save data/prep/20${evalyr}/hs_model_20${evalyr}, replace

	use data/prep/20${evalyr}/hs_model_20${evalyr}, clear
	...
	* historic year grades < 9
	...
	if (`v' == 12) {				
	preserve
		outsheet using data/prep/20${evalyr}/`v'th`up'th_model_only_20${evalyr}.csv, c replace
	restore
	}
	...
	preserve
		...
		foreach var of varlist(scoreMA_H1 scoreMA_E scoreVE_H1 scoreVE_E scoreaEN_H1 ///
		scoreaEN_E scoreaMA_H1 scoreaMA_E scoreaRD_H1 scoreaRD_E){
		keep if  `var'!= .
		}
		outsheet using data/prep/20${evalyr}/ACT_SAT_GPA_equating_20${evalyr}.csv, c replace
	restore
	...
	restore
		...
		drop if startyear_grade_E ==. 
		//percentEnrolledDays_1011 < .65 ??regardless of how received them??
		outsheet using data/prep/20${evalyr}/`v'th`up'th_model_only_20${evalyr}.csv, c replace
...
use data/orig/20${evalyr}/histyr_hs, clear
	...
	merge 1:1 id using data/prep/20${evalyr}/discipline_enroll_${evalyr}_metric_final_hs, nogen keep (1 3)
	merge 1:1 id using data/prep/20${evalyr}/crct_final_hs, nogen keep (1 3)
	merge 1:1 id using data/prep/20${evalyr}/mobility${histyr2}${histyr1}_hs, nogen keep (1 3)
	merge 1:1 id using data/prep/20${evalyr}/yearstograd, nogen keep(1 3)
	...
	* historic year grades < 9
	...
	restore
		...
		drop if startyear_grade_E ==. //percentEnrolledDays_1011 < .65 ??regardless of how received them??
		outsheet using data/prep/20${evalyr}/`v'th`up'th_model_only_20${evalyr}.csv, c replace
...
* SCORED DATASET
*****************
	use data/orig/20${evalyr}/yr13, nogen keep (1 3) // to confirm attendance for new group
	merge 1:1 id using data/prep/20${evalyr}/discipline_enroll_${histyr1}_metric_final, nogen keep (1 3)
	merge 1:1 id using data/prep/20${evalyr}/crct_final_2011, nogen keep (1 3)
	merge 1:1 id using data/prep/20${evalyr}/mobility${histyr2}${histyr1}_hs, nogen keep (1 3)
	merge 1:1 id using data/prep/20${evalyr}/crctSuccess1213, nogen keep(1 3)
