...
**************************
	...
	#delimit cr
		...
		rename (sch_code loc) (SchoolNumb loc_${histyr4}${histyr3})
...
rename id_${histyr3}${histyr2} id
${save} data/orig/absence_histyr2, replace
...
**************************************************
	...
	#delimit ;
		odbc load, clear exec	("
			SELECT [SCHOOLNUM]
			  ...
			  WHERE TRANSYEAR in (${histyr3} - 1) and
					EFFDATE < 20${histyr3}0530 and
					EFFDATE >= 20${histyr4}0801
								")
		dsn(ODS_Prod_MA) user(Research) pass(Research) ;
	#delimit cr
	...
	* now get summary by school vs. enrollment period
	...
	preserve
		...
		gen mob${histyr4}${histyr3} = num_enr/enr_days
		* save in place of mobility from Chris Lanzi attendance query
		save data\prep\sasi_mobility${histyr4}${histyr3}_hs, replace
	restore
		...
		* randomly remove one if same days at more than one school
		...
		foreach var of varlist loc dur { 
			rename `var' `var'_${histyr4}${histyr3}
		}
					
		isid id
		merge 1:1 id using `zoned1', nogen keep(1 3)
${save} data/orig/fay_histyr2_hs, replace
...
#delimit ;
		odbc load, clear exec	("
			...
			WHERE 	LEAVEDATE is not NULL and 
					LEAVECODE != '' and 
					LEN(PERMNUM) >= 7 and
					LEAVEDATE <= 20${histyr2}0801 and
					LEAVEDATE >= 20${histyr3}0801
										")
		dsn(ODS_Prod_MA) user(Research) pass(Research) ;
	#delimit cr
...
*******************************************
	...
	restore
		...
		gen gft_from8th_H3 = giftedind > 0 & giftedind != .
		drop repeatinggrd giftedind
...
****************************************************
	...
	*select the date to school calendar linking table
	...
	foreach yr in 20$histyr3 20$histyr2 {
		#delimit ;
			odbc load, clear exec	("
				SELECT 	Date, 
						SchoolDay 
...
/*		preserve
		...
		preserve
			...
			gen mob${histyr4}${histyr3}=(num_enr-1)/enr_days //single enrollment
			kdensity mob${histyr4}${histyr3} if mob${histyr4}${histyr3}!=0
			isid id
		restore
	!!CHANGED AT REQUEST OF IMT TO USE GSDR.GEMS.SASI_AENR */
use data\prep\sasi_mobility${histyr4}${histyr3}_hs, clear
	
${save} data\prep\mobility${histyr4}${histyr3}_hs, replace
	...
	#delimit ;
		odbc load, clear exec	("
			...
			WHERE 	EXAM_ADMIN_DATE > 20${histyr5}0201 and 
					EXAM_ADMIN_DATE < 20${histyr2}0801 and SUBJECT!= 'SS'
								")
...
*********************************
	#delimit ;
		odbc load, clear exec("
			...
			WHERE 	SchoolYear in (20${histyr3}, 20${histyr2}, 20${nxtyr}, 2014)")
		dsn(ODS_Prod_PA) user(Research) pass(Research);
	#delimit cr
		foreach var of varlist* { 
			destring `var', replace
...
*********************************
	...
	#delimit cr
		...
		keep if regexm(test_name, "RET") != 1   // use retakes next year?
			...
			isid id subject
${save} data/orig/gwy${histyr2}, replace */
	...
	#delimit ;
		odbc load, clear exec	("
			...
			WHERE		t2.[Date9thGrade] is not null and 
						t2.[Date9thGrade] > DATEADD(YEAR, -(${yrsbk}+3), CONVERT(datetime,convert(char(8), 20${evalyr}0801))) and 
						t2.[Date9thGrade] < DATEADD(YEAR, -(${yrsbk}-2), CONVERT(datetime,convert(char(8), 20${evalyr}0801))) and
						LEN(t1.[PermNum]) >= 7
		...
		odbc load, clear exec("
			SELECT [loc]
			  ...
			  WHERE season = 'Spring' and
					school_year in (20${histyr5}, 20${histyr4}, 20${histyr3}) or
					season = 'Fall' and school_year in (20${histyr4}, 20${histyr5}, 
					20${histyr4})
							")
		dsn(ODS_Prod_Assm) user(Research) pass(Research);
	#delimit cr
...
preserve
	...
	rename loc loc_${histyr4}${histyr3}
...
WHERE EXAM_ADMIN_DATE >= 20${histyr5}0801 and
	  EXAM_ADMIN_DATE <= 20${histyr2}0731 
")
		dsn(ODS_Prod_Assm) user(Research) pass(Research);
	#delimit cr
	
	${outsheet} using data/orig/act${histyr4}${histyr2}.csv, c replace
...
preserve
	foreach su in 9TH AME BIO ECON ALG GEOM PHY USH {
		import excel using /// 
		"`crt_pth'\20${histyr4}\EOCT_SP20${histyr4}_State_Summaries\SP${histyr4}_state_`su' with SD.xls", ///
			sheet("EOCT_SP08_`su'_State") cellrange(B2:G3) firstrow clear
				keep MeanScaleScore StDev
				rename(MeanScaleScore StDev) ///
					  (mn sd) 
			gen subject = "`su'"
			gen year = 20${histyr4}
			tempfile eoct_`su'_ga_$histyr4
			save `eoct_`su'_ga_$histyr4', replace
		}
	             use `eoct_9TH_ga_$histyr4', clear
		append using `eoct_AME_ga_$histyr4'
		append using `eoct_BIO_ga_$histyr4'
		append using `eoct_ECON_ga_$histyr4'
		append using `eoct_ALG_ga_$histyr4'
		append using `eoct_GEOM_ga_$histyr4'
		append using `eoct_PHY_ga_$histyr4'
		append using `eoct_USH_ga_$histyr4'
		

		replace subject = "AME" if subject == "AMLIT"
		replace subject = "ECO" if subject == "ECON"

	tempfile eoct_mean_sd_$histyr4
	save `eoct_mean_sd_$histyr4', replace
	
restore

* historic year 3
	***************
		* set path  EOCT_SP10_AMLIT_State
		local crt_pth = "${path}\RBES\" +  ///
		"Description\Original_DOE-CRCT-EOCT-HSGT_Mns-SDs"
		* bring in values from GA DOE  *MAI MAII(not in 2009 EOCT) 
preserve
	foreach su in ALG GEOM 9TH AME BIO ECON PHY USH {    
		import excel using /// 
		"`crt_pth'\20${histyr3}\EOCT_SP20${histyr3}_State_Summaries\SP${histyr3}_state_`su' with SD.xls", ///
			sheet("EOCT_SP09_`su'_State") cellrange(A2:H3) firstrow clear
				keep MeanScaleScore StDev
				rename(MeanScaleScore StDev) ///
					  (mn sd) 
			gen subject = "`su'"
			gen year = 20${histyr3}
			tempfile eoct_`su'_ga_$histyr3
			save `eoct_`su'_ga_$histyr3', replace
		}
				 use `eoct_9TH_ga_$histyr3', clear
		append using `eoct_AME_ga_$histyr3'
		append using `eoct_BIO_ga_$histyr3'
		append using `eoct_ECON_ga_$histyr3'
		append using `eoct_ALG_ga_$histyr3'
		append using `eoct_GEOM_ga_$histyr3'
		append using `eoct_PHY_ga_$histyr3'
		append using `eoct_USH_ga_$histyr3'
		
		
		replace subject = "ECO" if subject == "ECON"

	tempfile eoct_mean_sd_$histyr3
	save `eoct_mean_sd_$histyr3', replace
	
restore

* evaluation year (Histyr 2 - for 10th/11th grade)
	***************
* historic year 2
	***************
		* set path
...
preserve
	foreach su in 9TH AMLIT BIO ECO MAI MAII PHY USH {
		import excel using /// 
		"`crt_pth'\2010\State with SD only\SP10_state_`su' with SD.xls", ///
...
use `eoct_mean_sd_$histyr2', clear
append using `eoct_mean_sd_$histyr3'
append using `eoct_mean_sd_$histyr4'
...
save data/prep/eoct${histyr4}${histyr2}, replace
		...
		odbc load, clear exec("
						...
						SELECT [STUNUMB]
						  ...
						  WHERE EXAM_ADMIN_DATE >= 20${histyr4}0801 and
								EXAM_ADMIN_DATE <= 20${histyr2}0531 and
...
${save} data/prep/act${histyr4}${histyr2}, replace
...
odbc load, clear exec("
						...
						SELECT [STUNUMB]
						  ...
						  WHERE EXAM_ADMIN_DATE <= 20${histyr3}0531 and
					...
					outsheet using data/orig/psat${histyr4}${histyr3}.csv, c replace
		...
		* collReady variable based upon collegeBoard grade-specific thresholds
		...
		replace psat_collReady = 1 if 	psat_scoreCR != . & psat_scoreCR >= 45 & ///
			...
			drop notTenth
			isid id
		save data/prep/psat${histyr4}${histyr3}, replace
...
odbc load, clear exec("
					...
					outsheet using data/orig/sat${histyr4}${histyr2}.csv, c replace
		...
		save data/prep/sat${histyr4}${histyr2}, replace
...
*!! schoolyear is fall of academic year !!*
	...
	use data/prep/yearstograd, clear
		...
		* compute expected grade for use below
		...
		local hy6 = ${histyr6} //06
		local hy5 = ${histyr5} //07
		local hy4 = ${histyr4} //08
		local hy3 = ${histyr3} //09


		
		foreach yr in `hy6' `hy5' `hy4' `hy3' { 
...
preserve
	...
	********************************
	collapse (sum) *fail, by(id)
		...
		tempfile from8tH4
		save `from8tH4', replace
		
	use `byYear2', clear
		keep if schoolyear == 20${histyr3}
			drop schoolyear
		merge 1:1 id using `from8tH4', nogen keep(1 3)
...
preserve
		keep if schoolyear <= 20${histyr2} & grade >= 9 // histyr4 b/c completed course are predictor
...
preserve
		keep if schoolyear <= 20${histyr3} & grade >= 9 // histyr4 b/c completed course are predictor
		...
		forvalues y = `gpaYr'(-1)`gpaYrHist' {
				local yminus = `y' - 1
					...
					restore
							...
							keep if schoolyear <= `y' //avoids courses beyond histyr or histyr2, respectively
					} //END IF gpaYr
			...
			foreach s in lacourse macourse core {
						...
						keep if schoolyear <= 20${histyr2} & grade >= 5 // gpa is predictor and outcome
					...
					if `y' == `gpaYrHist' {
						...
						keep if schoolyear == `gpaYr' //after extracting historical data, pare down to histyr2
...
keep id startyear_grade20${histyr2}  startyear_grade20${nxtyr}
* don't need KK or PK
	destring startyear_grade20${histyr2} startyear_grade20${nxtyr}, replace force
	drop if startyear_grade20${histyr2} == . & startyear_grade20${nxtyr} == .
...
use data/orig/histyr_hs, clear
	drop ageseptember1st //need age Sept variable from histyr2 (here 2011-12)
	merge 1:1 id using data/orig/histyr2_hs, nogen keep(1 3)
	merge 1:1 id using data/orig/absence_histyr2,nogen keep (1 3)
	merge 1:1 id using data/orig/absence_histyr,nogen keep (1 3)
	merge 1:1 id using data/prep/discipline_enroll_${histyr3}_metric_final_hs, nogen keep (1 3)
	merge 1:1 id using data/prep/discipline_enroll_${histyr2}_metric_final_hs, nogen keep (1 3)
	merge 1:1 id using data/prep/crct_final_hs, nogen keep (1 3)
	merge 1:1 id using data/prep/mobility${histyr4}${histyr3}_hs, nogen keep (1 3)
	merge 1:1 id using data/prep/yearstograd, nogen keep(1 3)
	
	
	//2011-12 GPA - 1Sem is outcome for 11th to 12th analyses
	merge 1:1 id using data/prep/core1SemGPA20${histyr3}20${histyr2}_hs, nogen keep(1 3)
		merge 1:1 id using data/prep/lacourse1SemGPA20${histyr3}20${histyr2}_hs, nogen keep(1 3)
			merge 1:1 id using data/prep/macourse1SemGPA20${histyr3}20${histyr2}_hs, nogen keep(1 3)
			
	//2011-12 GPA - Annual is 2011-12 model output
	merge 1:1 id using data/prep/coreAnnualGPA20${histyr3}20${histyr2}, nogen keep(1 3)
		merge 1:1 id using data/prep/lacourseAnnualGPA20${histyr3}20${histyr2}, nogen keep(1 3)
			merge 1:1 id using data/prep/macourseAnnualGPA20${histyr3}20${histyr2}, nogen keep(1 3)
			
	//2010-11 GPA - Cumul is 2011-12 model input
	merge 1:1 id using data/prep/coreCumulGPA20${histyr4}20${histyr3}, nogen keep(1 3)
		merge 1:1 id using data/prep/lacourseCumulGPA20${histyr4}20${histyr3}, nogen keep(1 3)
			merge 1:1 id using data/prep/macourseCumulGPA20${histyr4}20${histyr3}, nogen keep(1 3)

	merge 1:1 id using data/prep/sei, nogen keep(1 3)
	merge 1:1 id using data/prep/coursesTowardGrad, nogen keep(1 3)
	merge 1:1 id using data/prep/sat${histyr4}${histyr2}, nogen keep(1 3)
	merge 1:1 id using data/prep/act${histyr4}${histyr2}, nogen keep(1 3)
	merge 1:1 id using data/prep/eoct${histyr4}${histyr2}, nogen keep(1 3)

	merge 1:1 id using data/prep/promotion_hs, nogen keep(1 3)
	merge 1:1 id using data/orig/fay_histyr2_hs, nogen keep(1 3)
	merge 1:1 id using data/prep/failcore_rigorInd, nogen keep(1 3)
	merge 1:1 id using data/prep/census, nogen keep(1 3)
	merge 1:1 id using data/prep/psat${histyr4}${histyr3}, nogen keep(1 3)
	...
	* summarize prior crct score by school
	...
	restore
		...
		* make more nuanced SPED categories
		gen spedCatMin_H3 = inlist(specialeducationprimaryarea_${histyr4}${histyr3}, ///
		...
		gen spedCatMod_H3 = inlist(specialeducationprimaryarea_${histyr4}${histyr3}, ///					
					"Autism", "Emotional/Behavioral Disorder", ///
					"Mild Intellectual Disability", "Specific Learning Disability")
		gen spedCatSev_H3 = inlist(specialeducationprimaryarea_${histyr4}${histyr3}, ///
	...
	save data/prep/hs_model_${histyr3}${histyr2}, replace

	use data/prep/hs_model_${histyr3}${histyr2}, clear
	...
	*hist core_pseudo_gpa1011, by( grade_2011) addplot(pci 0 83 .05 83)
	...
	if (`v' == 12) {
	keep if grade_20${histyr3} == `v'
	}
	
	
	* historic year grades of 9-11
	if inlist(`v',9, 10, 11) {
	keep if grade_20${histyr3} == `v' & daysenrolled_${histyr3}${histyr2} !=.
	...
	* historic year grades < 9
	if (`v' < 9) {
	keep if grade_${histyr4}${histyr3} == `v'
		*drop grade_20${histyr3}
	}
	gen ontimeGrad = yearstograd == 4 & yearstograd != . & grad == 1

	drop 	schoolyear_${histyr3}${histyr2} fullname_${histyr3}${histyr2} /// 
			gender_${histyr3}${histyr2} birthdate_${histyr3}${histyr2} ///
			ethnicity_${histyr3}${histyr2} primarylanguage_${histyr3}${histyr2} ///
			ageseptember1st_${histyr3}${histyr2} freereducedlunch_${histyr3}${histyr2} ///
			lep_${histyr3}${histyr2} ell_${histyr3}${histyr2} ///
			specialeducationprimaryarea_${histyr3}${histyr2} /// 
			altschoolindicator_${histyr3}${histyr2} giftedindicator_${histyr3}${histyr2} ///
			repeatinggradeindicator_${histyr3}${histyr2} //daysabsent_${histyr3}${histyr2}

	gen pabs_${histyr4}${histyr3} = ///
			daysabsent_${histyr4}${histyr3}/daysenrolled_${histyr4}${histyr3}
	gen pabs_${histyr3}${histyr2} = ///
			daysabsent_${histyr3}${histyr2}/daysenrolled_${histyr3}${histyr2}
	gen punex_${histyr4}${histyr3} = ///
			daysunexcused_${histyr4}${histyr3}/daysenrolled_${histyr4}${histyr3}
	gen punex_${histyr3}${histyr2} = ///
			daysunexcused_${histyr3}${histyr2}/daysenrolled_${histyr3}${histyr2}
	gen percentEnrolledDays_${histyr4}${histyr3} = ///
			daysenrolled_${histyr4}${histyr3}/180
	gen sped_${histyr4}${histyr3} = specialeducation != " "

		order id-specialeducation sped_${histyr4}${histyr3}
	drop 	birthdate_${histyr4}${histyr3} ell_${histyr4}${histyr3} ///
			primarylanguage_${histyr4}${histyr3} altschool ///
			daysenrolled_${histyr4}${histyr3} daysabsent_${histyr4}${histyr3} disc_enr ///
			sequence iss_oss_days num_enr schoolyear grade_${histyr4}${histyr3} ///
			grade_${histyr3}${histyr2} specialeducation
		
	gen female if gender != "" = gender == "Female"

	gen black if ethnicity_${histyr4}${histyr3} != "" = ///
					ethnicity_${histyr4}${histyr3} == "B"
	gen hispanic if ethnicity_${histyr4}${histyr3} != "" = /// 
					ethnicity_${histyr4}${histyr3} == "H"
	gen other if ethnicity_${histyr4}${histyr3} != "" = /// 
					inlist(ethnicity_${histyr4}${histyr3}, "A", "I", "M")
	gen frl_${histyr4}${histyr3} if freereducedlunch_${histyr4}${histyr3} != "" = ///
					inlist(freereducedlunch_${histyr4}${histyr3}, "F", "R")
		rename lep_${histyr4}${histyr3} limitedenglishprof
	gen lep_${histyr4}${histyr3} if limitedenglishprof != "" = /// 
					inlist(limitedenglishprof, "Y", "M")
	gen repgrd_${histyr4}${histyr3} if repgrd != . = repgrd == 1
	gen gft_${histyr4}${histyr3} if gft != . = gft == 1
		...
		foreach var of varlist* {
			...
			//eval yr
			local ev =  "_${histyr3}${histyr2}|_20${histyr2}|_${histyr2}|" + ///
						"${histyr3}${histyr2}|20${histyr3}20${histyr2}|" + ///
						"20${histyr2}|${histyr2}"			
		...
		foreach var of varlist* {
			...
			//hist yr 1
			local hs1 =  "_${histyr4}${histyr3}|_20${histyr4}20${histyr3}|" + ///
						 "_20${histyr3}|_${histyr3}|${histyr4}${histyr3}|" + ///
						 "20${histyr4}20${histyr3}|20${histyr3}|${histyr3}"			
	
			rename `var' `=regexr("`var'", "`hs1'", "_H3")'
		}
		

		foreach var of varlist* {		
		//hist yr 2		
			rename `var' `=regexr("`var'", "20${histyr4}", "_H4")'
		}

		foreach var of varlist* {		
		//hist yr 3	
			rename `var' `=regexr("`var'", "20${histyr5}", "_H5")'
		}

	if (`v' == 12) {				
	preserve
		outsheet using data/prep/`v'th`up'th_model_only_${histyr3}${histyr2}.csv, c replace
	restore
	}
...
*****************
	...
	merge 1:1 id using data/prep/discipline_enroll_${histyr3}_metric_final, nogen keep (1 3)
	merge 1:1 id using data/prep/crct_final_2011, nogen keep (1 3)
	merge 1:1 id using data/prep/mobility${histyr4}${histyr3}_hs, nogen keep (1 3)
	merge 1:1 id using data/prep/crctSuccess1213, nogen keep(1 3)

		drop if ethnicity_${histyr4}${histyr3} == ""
		...
		* finalize dataset
		...
		replace wht=1 if ethnicity=="W"
			...
			tab blk ethnicity,m
				...
				gen pabs${histyr4}${histyr3}= daysabsent_${histyr4}${histyr3} / daysenrolled_${histyr4}${histyr3}
						...
						specialeducationprimaryarea=="Traumatic Brain Injury" 
							...
							replace female = 1 if trim(gender)=="Female"
								drop gender
									...
									rename altschoolindicator_${histyr4}${histyr3} alt${histyr4}${histyr3}_ind
									rename giftedindicator_${histyr4}${histyr3} gift${histyr4}${histyr3}_ind
									rename repeatinggradeindicator_${histyr4}${histyr3} repgrd${histyr4}${histyr3}_ind
							...
							local hyr3="$histyr3"
							local hyr4="$histyr4"
							foreach var in gift`hyr4'`hyr3' repgrd`hyr4'`hyr3' alt`hyr4'`hyr3' { // uses first part of var name
								tab `var',m
								gen `var' = 0
									...
									drop `var'_ind
										...
										*drop if grade=="PK" | grade=="KK"  // these cannot be matches anyway removed in extract in 2012-13
										...
										rename grade enrgrd
												...
												egen lmrscssz${histyr3}mn=rmean(la${histyr3}z ma${histyr3}z rd${histyr3}z sc${histyr3}z ss${histyr3}z)
												
														replace drate_${histyr4}${histyr3} = 0 if drate_${histyr4}${histyr3}==. & pabs${histyr4}${histyr3}!=.
														replace dsevmx_${histyr4}${histyr3} = 0 if dsevmx_${histyr4}${histyr3}==. & pabs${histyr4}${histyr3}!=.
														replace dsevmn_${histyr4}${histyr3} = 0 if dsevmn_${histyr4}${histyr3}==. & pabs${histyr4}${histyr3}!=. 
